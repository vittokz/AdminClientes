<?php require_once("../seguridad/seguridad.php");
   ...
   $identidad = $row["cedula_usuario"];
		...
		if($ban==0){
			echo "<CENTER><br><p style='font-size:18px; color:RED'>Nombre del Demandado no existe!!!</p></CENTER>";
...
<head>
  ...
  <title>.::Buscar por departamento::.</title>
...
<div class="wrapper">
  <div class="box">
   <div class="box-body table-responsive no-padding">
             ...
             <?php
			     //busco en departamentos el codigo dela ciudad
				  $sqlM=mysqli_query($conexion,"select id from lista_estados where relacion like '$depar'");
                     $idCiudad="";
          				 while($rowM = mysqli_fetch_assoc($sqlM)){	
					        $idCiudad = $rowM["id"];
				          
							  //busco en juzgados el codigo del juzgafo segun la ciudad
							  $sqlJ=mysqli_query($conexion,"SELECT * FROM des_juzgados where ciudad like '$idCiudad'");
								 while($rowJ = mysqli_fetch_assoc($sqlJ)){
										$idJuz =$rowJ["idJuzgado"];
										 //recojo datos del proceso
										$sqlProceso=mysqli_query($conexion,"SELECT * FROM des_procesos WHERE idJuzgado like '$idJuz' and idCliente like '$identidad'");
							...
							?>
							  <tr>
                                  ...
                                  <a href="procesoSeleccionado.php?idProceso=<?php echo $idProceso;?>&identidad=<?php echo $idCliente;?>">
